(function($) {
    ...
    GetList.prototype = {
        ...
        BuildHead: function() {
            ...
            //head头部
            var orderlist = [
                { key: '法院层级', value: '法院层级' }, { key: '裁判日期', value: '裁判日期' }, { key: '审判程序', value: '审判程序' }
            ];
            for (var i = 0; i < orderlist.length; i++) {
                ...
                var $divSort;
                if (i == 0) {
                    $divSort = $("<div class=\"buttonclick\" key='desc'>" +
                        value +
                        "<div class=\"descico\"></div></div>");
                } else {
            ...
            //start add  kingirvin 2015-10-13
            ...
            $divListDownLoad.click(function() {
                ...
                if (getListDocIds.length > 0) {
                    ...
                    getListDocIds.each(function() {
                        DownloadDocIds.push($(this).attr("downloadValue"));
                    });
            ...
            var $divListCollect = $("<div class='list-operate'><span>批量收藏</span></div>");
                    ...
                    success: function(res) {
                        if (res == "0") {
                            ...
                            if (ckChecked.length > 0) {
                                ckChecked.each(function() {
                                    caseInfo = caseInfo + $(this).attr("value") + "&";
                                });
            ...
            //总条数
            var $divListDataCount =
                $(
                    "<div class=\"list_datacount\">共找到<span id='span_datacount' style='color:red;'>0</span>个结果<div class=\"downloadico\"></div></div>");
        ...
        BuildList: function(obj, index, page, type) {
            var s = this;
            var $this = $(this.element);
            var param = $(this.setting.param);
            var datalist = "";
            ...
            list_center.html(
                "<div style='height:25px;line-height:25px;vertical-align:middle;'><img src='/Assets/js/libs/icons/loading.gif'/><span>正在加载，请稍候...</span></div>");
            //center中部数据
            //1.取数据
            var listparam = "";
                    ...
                    if (datalist != undefined && datalist != null) {
                        if (datalist.length > 1) {
                            ...
                            for (var i = 1; i < datalist.length; i++) {
                                ...
                                relateFiles = (relateFiles == "" ? relateFiles : (relateFiles + "~")) +
                                    key +
                                    "|" +
                                    caseCourt +
                                    "|" +
                            ...
                            for (var i = 1; i < datalist.length; i++) {
                                var key = datalist[i].文书ID != undefined ? datalist[i].文书ID : "";
                                var caseNameTitle = datalist[i].案件名称 != undefined
                                    ? datalist[i].案件名称.replace(",", "，")
                                    : "";
                                currentDocId += key + ",";
                                title = caseNameTitle;
                                //add by zhs 151213 案件名称为空的数据 不显示
                                if (title == "") {
                                    continue;
                                }
                                //var titleOri = datalist[i].案件名称 != undefined ? datalist[i].案件名称 : "";
                                titleOri = caseNameTitle; //title和titleOri？？？
                                var caseTypeInt = datalist[i].案件类型 != undefined ? datalist[i].案件类型 : "";
                                ...
                                var today = new Date();
                                //var dateNow = today.getFullYear() + "-" + (today.getMonth() + 1) + "-" + ("00" + today.getDate().toString()).substr((today.getDate().toString()).length);
                                ...
                                if (keyWordArr.length > 0) {
                                    ...
                                    if (caseContent.indexOf(keyWord) >= 0) {
                                        ...
                                        if (startIndex <= 0) {
                                            ...
                                            markRedCon = markRedCon +
                                                caseContent.substr(keyWordIndex + keyWord.length, 130 - keyWordIndex);
                                        } else {
                                            markRedCon = markRedCon +
                                                caseContent.substr(keyWordIndex - 60, 60) +
                                                keyWord;
                                            markRedCon = markRedCon +
                                                caseContent.substr(keyWordIndex + keyWord.length, 60);
                                        }
                                ...
                                linkA = encodeURI(linkA);
                                var li = $("<div class=\"dataItem\">" +
                                    "<div class=\"label\">" +
                                    (caseType != "" ? ("<div class=\"ajlx_lable\">" + caseType + "</div>") : "") +
                                    (trialRound != ""
                                        ? ("<div class=\"ajlx_lable\" style=\"" + style + "\">" + trialRound + "</div>")
                                        : "") +
                                    "</div>" +
                                    "<table>" +
                                    "<tr>" +
                                    ...
                                    //start add  kingirvin 2015-10-13
                                    +
                                    "<input type=\"hidden\" class=\"DocIds\" value=\"" +
                                    key +
                                    "|" +
                                    titleOri +
                                    "|" +
                                    ...
                                    //end  add  kingirvin 2015-10-13
                                    +
                                    "<input class='listck' type='checkbox' name='ckList' downloadValue=\"" +
                                    key +
                                    "|" +
                                    titleOri +
                                    "|" +
                                    ...
                                    "<div class=\"scxz\">" +
                                    "<div class=\"download\">" +
                                    "<img style=\"cursor:pointer; margin-bottom:5px;\" title='通过中国司法案例网身份认证的会员（主要包括法官、律师、法律学者、法律院校学生）才能推荐裁判文书到中国司法案例网，经全体会员众筹投票，符合条件可收入最高人民法院司法案例库' onclick=\"Casefx('" +
                                    key +
                                    "');\" src=\"/Assets/img/list/wytal-hove.png\" alt=\"通过中国司法案例网身份认证的会员（主要包括法官、律师、法律学者、法律院校学生）才能推荐裁判文书到中国司法案例网，经全体会员众筹投票，符合条件可收入最高人民法院司法案例库\"/>" +
                                    "</div>" +
                                    "<div class=\"download\">" +
                                    "<img style=\"cursor:pointer;\" title='下载' onclick=\"DownLoadCase('" +
                                    key +
                                    "|" +
                                    title.replace('<span style=\'color:red\'>', '').replace('</span>', '') +
                                    "|" +
                                    judgeDate +
                                    "');\" src=\"/Assets/img/list/list_download.png\" alt=\"下载\"/>" +
                                    "</div>" +
                                    "<div class=\"collect\">" +
                                    "<img style=\"cursor:pointer;\" title='收藏'  onclick=\"CollectCase('" +
                                    key +
                                    "','" +
                                    title.replace('<span style=\'color:red\'>', '').replace('</span>', '') +
                                    "','" +
                                    ...
                                    "','" +
                                    judgeDate +
                                    "');\" src=\"/Assets/img/list/list_collect.png\"  alt=\"收藏\"/>" +
                                    "</div>" +
                                    "</div>" +
                                    "</div>");
        ...
        BuildBottom: function(obj, dataCount) {
            ...
            list_bottom.html("");
            //分页bottom
            var $pager = $('<div style="margin-top:20px;" id="pageNumber" class="pageNumber" total="' +
                dataCount +
                '"  pageSize="5" showSelect="true" selectDirection="bottom" centerPageNum="5" edgePageNum="0"  prevText="上一页" nextText="下一页" selectData=\'{"list":[{"key":5,"value":5},{"key":10,"value":10},{"key":15,"value":15},{"key":20,"value":20}]}\'></div>');
            $pager.attr("total", dataCount);
            list_bottom.append($pager);
            $pager.render();
            $pager.bind("pageChange",
                function(e, index) {
                    var pageSize = $(this).attr('pageSize');
                    obj.BuildList(obj, index + 1, pageSize, 0);
                    $("html,body").animate({ scrollTop: 0 }, 'fast');
                });
            $pager.bind("sizeChange",
                function(e, num) {
                    obj.BuildList(obj, 1, num, 0);
                });
        },
                        ...
                        if (relateFile.length > 0) {
                            ...
                            for (var ii = 0; ii < relateFile.length; ii++) {
                                ...
                                if (arry_arrentId[i] == relateFile[ii].文书ID) {
                                    if (ii == 0) {
                                        img +=
                                            "<img src=\"/Assets/img/list/bp_18.png\" style=\"padding-left:2px;\"/><img style=\"padding-bottom: 5px;\" src=\"/Assets/img/list/dot_10.png\"/>";
                                    } else if (ii == relateFile.length - 1 && relateFile.length == 2) {
                                        img +=
                                            "<img style=\"padding-left:2px;\" src=\"/Assets/img/list/bp_18.png\"/><img style=\"margin-bottom: 10px;margin-right: 5px;\" src=\"/Assets/img/list/dot_04.png\"/>";
                                    } else if (ii < relateFile.length - 1) {
                                        img +=
                                            "<img style=\"margin-bottom:10px;\" src=\"/Assets/img/list/bp_18.png\"/><img src=\"/Assets/img/list/dotLine_03.png\"/>";
                                    } else {
                                        img +=
                                            "<img src=\"/Assets/img/list/bp_18.png\" style=\"padding-left:2px;\" /><img style=\"padding-right: 4px;padding-bottom: 11px;\" src=\"/Assets/img/list/dot_07.png\"/>";
                                    }

                                } else {
                                    if (ii == 0) {
                                        img +=
                                            "<img style=\"padding-bottom: 5px;margin-left:10px;margin-top: 10px;\" src=\"/Assets/img/list/dot_10.png\"/>";
                                    } else if (ii < relateFile.length - 1) {
                                        img +=
                                            "<img style=\"margin-left:45px\" src=\"/Assets/img/list/dotLine_03.png\"/>";
                                    } else if (ii == relateFile.length - 1 && relateFile.length == 2) {
                                        img +=
                                            "<img style=\"margin-left:48px;margin-right: 5px;\" src=\"/Assets/img/list/dot_04.png\"/>";
                                    } else {
                                        img +=
                                            "<img style=\"padding-right: 4px;padding-top: 8px;\" src=\"/Assets/img/list/dot_07.png\"/>";
                                    }
                                }
                            }
                            for (var ii = 0; ii < relateFile.length; ii++) {
                                ...
                                html += "<lable title=\"关联文书仅以本网收录的裁判文书为依据，如果对此结果有疑义，请联系作出裁判文书的人民法院。\">" +
                                    title +
                                    "</lable>";
                                html += "</td>";
                                html += "</tr>";
                                ...
                                if (ii == 0) {
                                    html += "<td  rowspan=" +
                                        colStr +
                                        " style=\"text-align: right;\"><div style=\"width: 70px;float: right; margin-bottom: 10px;\">" +
                                        img +
                                        "</div></td>";
                                }
                                if (arry_arrentId[i] == relateFile[ii].文书ID) {
                                    ...
                                    html += "&nbsp;" + relateFile[ii].审理法院 + "";
                                    html +=
                                        "&nbsp;<a style='text-decoration:underline;' title='点击查看全文' target='_blank' href=\"/Content/Content?DocID=" +
                                        relateFile[ii].文书ID +
                                        "\">" +
                                        relateFile[ii].案号 +
                                        "</a>";
